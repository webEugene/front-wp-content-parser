    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-8 py-10 max-sm:max-w-sm max-sm:mx-auto gap-y-8">
        <div class="col-span-full mb-10 lg:col-span-2 lg:mb-0">
          ...
          <nuxt-link to="/"  class="py-2.5 px-5 h-9 block w-fit bg-indigo-600 rounded-full shadow-sm text-xs text-white mx-auto transition-all  duration-500 hover:bg-indigo-700 lg:mx-0">
            Contact us
          </nuxt-link>
        </div>
        <!--End Col-->
        <div class="lg:mx-auto text-left ">
          ...
          <ul class="text-sm  transition-all duration-500">
            <li v-for="link in homepageList" :key="link.title" class="mb-4 last:mb-0">
              <nuxt-link :to="link.url" :aria-label="link.label" class="text-gray-600 hover:text-gray-900">{{link.title}}</nuxt-link>
            </li>
          </ul>
        </div>
        <!--End Col-->
        <div class="lg:mx-auto text-left ">
          ...
          <ul class="text-sm  transition-all duration-500">
            <li v-for="link in freeProductsList" :key="link.title" class="mb-4 last:mb-0">
              <nuxt-link :to="link.url" :aria-label="link.label" class="text-gray-600 hover:text-gray-900">{{link.title}}</nuxt-link>
            </li>
          </ul>
        </div>
        ...
        <div class="lg:mx-auto text-left">
          ...
          <ul class="text-sm  transition-all duration-500">
            <li v-for="link in companyList" :key="link.title" class="mb-4 last:mb-0">
              <nuxt-link :to="link.url" :aria-label="link.label" class="text-gray-600 hover:text-gray-900">{{link.title}}</nuxt-link>
            </li>
          </ul>
        </div>
      ...
      <div class="py-7 border-t border-gray-200">
        <div class="flex items-center justify-center flex-col lg:justify-between lg:flex-row">
          <span class="text-sm text-gray-500 "><nuxt-link to="/">Wp Parser</nuxt-link> © {{ new Date().getFullYear() }}, All rights reserved.</span>
